...
vec4 PackData(vec3 Normal, vec3 Diffuse, float Depth) {
	...
	Packed.w = uintBitsToFloat(packHalf2x16(vec2(Depth, 0.0))); 
...
void main() {
	...
	for(int x = -3; x <= 3; x++) {
		...
		if(x != 0) {
			...
			if(abs(SampleDepth-Depth) < mix(0.5,2.0,pow(1.-abs(dot(Normal.xyz, IncidentVector)),2.))) {
				
				float SampleWeight = pow(max(dot(SampleNormal, Normal),0.0), 2.0) * (1.0 / (abs(x)+5.0)); 
